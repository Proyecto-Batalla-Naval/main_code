...
class JuegoCPU:
    def __init__(self):
        self.barcos_cpu = []
        self.disparos_cpu = []
        self.max_disparos = 18  # <- AÑADIR ESTA LÍNEA
        self.disparos_restantes = 18  # <- AÑADIR ESTA LÍNEA
...
def JuegoIndividual(posiciones_jugador, datos_jugador):
    ...
    mensaje = ""
    mensaje_tiempo = 0
    max_disparos = 18  # <- AÑADIR ESTA LÍNEA
    disparos_restantes_jugador = 18  # <- AÑADIR ESTA LÍNEA
    disparos_restantes_cpu = 18  # <- AÑADIR ESTA LÍNEA    
    ...
    while juego_activo:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and turno_jugador and disparos_restantes_jugador > 0:
                pos = pygame.mouse.get_pos()
        ...
        # Turno de la CPU
        if not turno_jugador and disparos_restantes_cpu > 0 and juego_activo:
            time.sleep(1)
            ...
            # Dentro de JuegoIndividual(), en la sección de verificación de victoria
            ...
            if jugador_gana:
                ...
                mensaje_ganador = "¡HAS PERDIDO!"
                juego_activo = False
            elif disparos_restantes_jugador <= 0 and not jugador_gana:
                mensaje_ganador = "HAS PERDIDO - Sin disparos restantes"
                juego_activo = False
            elif disparos_restantes_cpu <= 0 and not cpu_gana:
                mensaje_ganador = "¡HAS GANADO! - CPU sin disparos"
                juego_activo = False
        ...
        ventana.blit(texto_turno, (ancho//2 - texto_turno.get_width()//2, alto - 50))
        texto_disparos_jug = Fuente_opcion.render(f"Tus disparos: {disparos_restantes_jugador}", True, azul)
        ventana.blit(texto_disparos_jug, (ancho - 250, 20))
        texto_disparos_cpu = Fuente_opcion.render(f"Disparos CPU: {disparos_restantes_cpu}", True, rojo)
        ventana.blit(texto_disparos_cpu, (ancho - 250, 60))        
        pygame.display.flip()
        reloj.tick(30)
        if not juego_activo:
    # REEMPLAZAR TODO LO QUE HAYA AQUÍ CON:
            ventana.blit(fondo2, (0, 0))
    
    # Mensaje principal
            texto_ganador = Fuente_Principal.render(mensaje_ganador, True, verde if "GANADO" in mensaje_ganador else rojo)
            ventana.blit(texto_ganador, (ancho//2 - texto_ganador.get_width()//2, alto//2))
    
    # Estadísticas de disparos
            texto_stats = Fuente_opcion.render(
                f"Tus disparos: {18 - disparos_restantes_jugador}/18 | CPU: {18 - disparos_restantes_cpu}/18", 
                True, 
                negro
            )
            ventana.blit(texto_stats, (ancho//2 - texto_stats.get_width()//2, alto//2 + 50))
